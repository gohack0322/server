...
import OnIceCandidateHandler from '@/utils/client/OnIceCandidateHandler'
import TextReceiver from '@/utils/client/text/receiver'
...
export default class MultiPeers {
    ...
    createNewPeer(remoteUserId, userPreferences) {
        if (this.rtc.maxParticipantsAllowed <= this.rtc.getAllParticipants().length) {
            return;
        }

        userPreferences = userPreferences || {};

        if (this.rtc.isInitiator && !!this.rtc.session.audio && this.rtc.session.audio === 'two-way' && !userPreferences.streamsToShare) {
            userPreferences.isOneWay = false;
            userPreferences.isDataOnly = false;
            userPreferences.session = this.rtc.session;
        }
        ...
        userPreferences = this.rtc.setUserPreferences(userPreferences, remoteUserId);
        var localConfig = this.getLocalConfig(null, remoteUserId, userPreferences);
        this.rtc.peers[remoteUserId] = new PeerInitiator(localConfig);
    }
