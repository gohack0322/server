...
import CodecsHandler from '@/utils/client/CodecsHandler'
import IceServersHandler from '@/utils/client/IceServersHandler'
import StreamsHandler from '@/utils/client/StreamsHandler'
import TranslationHandler from '@/utils/client/TranslationHandler'
import FileProgressBarHandler from '@/utils/client/FileProgressBarHandler'
import { currentUserMediaRequest, getUserMediaHandler } from '@/utils/client/getUserMediaHandler'
import { getRandomString, getRMCMediaElement, listenEventHandler, getTracks, isData, isUnifiedPlanSupportedDefault } from '@/utils/client/globals'
import '@/utils/client/ios-hacks'
import { RTCPeerConnection } from '@/utils/client/RTCPeerConnection'
import MultiPeers from '@/utils/client/MultiPeers'
import SocketConnection from '@/utils/client/SocketConnection'
...
class RTCMultiConnection {
    constructor(roomid, forceOptions) {
        ...
        this.channel = this.sessionid = (roomid || location.href.replace(/\/|:|#|\?|\$|\^|%|\.|`|~|!|\+|@|\[|\||]|\|*. /g, '').split('\n').join('').split('\r').join('')) + '';

        this.socketAutoReConnect = true
        this.enableLogs = true

        var mPeer = new MultiPeers(this)
        ...
        mPeer.onNegotiationNeeded = (message, remoteUserId, callback) => {
            callback = callback || function() {};
            ...
            this.connectSocket(() => {
                this.socket.emit(this.socketMessageEvent, messageToDeliver, callback);
            });
        };
        ...
        connection.openOrJoin = function(roomid, callback) {
            ...
            connection.checkPresence(roomid, function(isRoomExist, roomid) {
                if (isRoomExist) {
                    ...
                    beforeJoin(connectionDescription.message, () => joinRoom(connectionDescription, callback));
                    return;
                }
        ...
        connection.open = (roomid, callback) => {
            callback = callback || function() {};
        ...
        connection.join = (remoteUserId, options) => {
        ...
        connection.checkPresence = (roomid, callback) => {
            ...
            if (!connection.socket) {
                this.connectSocket(() => {
                    this.checkPresence(roomid, callback);
                });
                return;
            }
        ...
        connection.onReadyForOffer = function(remoteUserId, userPreferences) {
            connection.multiPeers.createNewPeer(remoteUserId, userPreferences);
        };
    ...
    connectSocket(connectCallback) {
        this.socketAutoReConnect = true;

        if (this.socket) { // todo: check here readySate/etc. to make sure socket is still opened
            if (connectCallback) {
                connectCallback(this.socket);
            }
            return;
        }
        ...
        new SocketConnection(this, socket => {
            if (connectCallback) {
                connectCallback(this.socket);
            }
        });
    ...
    closeSocket() {
        ...
        if (typeof this.socket.resetProps === 'function') {
            this.socket.resetProps();
        }
    ...
    onSocketError(event) {
        if (this.enableLogs) {
            console.warn('socket.io connection is failed', event);
        }
    }
}
